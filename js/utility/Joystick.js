...
function handleJoystickControls() {
    ...
    if (!navigator.getGamepads) return; // old browser?
    var gamepad = navigator.getGamepads()[0]; // first avail
    if (!gamepad) return; // note: always null unless user has pressed a button
    ...
    // game-specific control logic
    if (isJoystickBeingUsed &&
        window.vehicleList && 
        window.vehicleList.length) { 
